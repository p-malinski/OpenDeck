<script lang="ts">
    ...
    async function iframeOnLoad(instance: ActionInstance) {
        ...
        if (split[2] == "Encoder") {
            ...
            coordinates = {
                row: Math.floor(position / device.columns),
                column: position % device.columns,
            };
        }
        ...
        if (
            instance == null ||
            !iframe.src ||
            !iframe.src.startsWith(getWebserverUrl())
        )
            return;
        const info = JSON.stringify(
            await invoke("make_info", { plugin: instance.action.plugin })
        );
    ...
    const closePopup = (context: string) => {
        ...
        if (iframe) {
            ...
            iframe.style.display =
                $inspectedInstance == context ? "block" : "none";
            iframe.contentWindow?.postMessage(
                { event: "windowClosed" },
                getWebserverUrl()
            );
            ...
            window
                ...
                .catch((error: any) => {
                    iframes[data.payload.context]?.contentWindow?.postMessage(
                        {
                            event: "fetchError",
                            payload: { id: data.payload.id, error },
                        },
                        getWebserverUrl()
                    );
                });
    ...
    $: instances = profile.keys
        ...
        .reduce(
            (prev, current) =>
                prev.concat(
                    current.children ? [current, ...current.children] : current
                ),
            [] as ActionInstance[]
        )
    ...
    listen("plugin_reloaded", ({ payload }: { payload: string }) => {
        for (const instance of instances) {
            if (
                instance.action.plugin == payload &&
                iframes[instance.context]
            ) {
...
<div
    class="grow overflow-auto bg-white dark:bg-neutral-900 border-t dark:border-neutral-700"
